...
export default function AccountModal({ customer, onClose, onLogout, onUpdatePoints, currency }) {
  ...
  const statusColor = { pending: '#f59e0b', processing: '#0077B6', shipped: '#059669', delivered: '#059669', cancelled: '#ef4444' }
  ...
  return (
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet">
        ...
        {/* Header */}
        <div style={{ background: 'linear-gradient(150deg,#005F92,#0077B6 55%,#059669)', padding: '22px 18px 20px', position: 'relative' }}>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', borderBottom: '2px solid #EFF6FF', background: 'white', position: 'sticky', top: 0, zIndex: 2 }}>
          {[{ id: 'info', label: '👤 بياناتي' }, { id: 'orders', label: '📦 طلباتي' }].map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ flex: 1, padding: '12px', border: 'none', background: 'none', fontFamily: 'inherit', fontWeight: 800, fontSize: 14, cursor: 'pointer', color: activeTab === t.id ? '#0077B6' : '#94a3b8', borderBottom: activeTab === t.id ? '3px solid #0077B6' : '3px solid transparent', transition: '.2s' }}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'info' && (
            <div>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid #EFF6FF' }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: '#EFF6FF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>{item.icon}</div>
                  <div>
              ...
              {loading
                ...
                : orders.length === 0
                  ...
                  : orders.map(ord => {
                      ...
                      return (
                        <div key={ord.id} style={{ border: '1.5px solid #EFF6FF', borderRadius: 16, padding: 14, marginBottom: 12 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                            <div style={{ fontWeight: 800, fontSize: 13, color: '#1E293B' }}>طلب #{ord.id}</div>
                          ...
                          {items.slice(0, 2).map((it, i) => (
                            <div key={i} style={{ fontSize: 12, color: '#0077B6', display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                              <span>📦</span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, borderTop: '1px solid #EFF6FF', paddingTop: 10 }}>
                            <span style={{ fontWeight: 900, color: '#0077B6', fontSize: 16 }}>{parseFloat(ord.total).toFixed(0)} {currency}</span>
