 ...
 * ✅ الطلب بالكارتون فقط
 * ✅ نقاط الولاء
 * ✅ الطلب السريع (Quick Order)
 * ✅ الطلبات السابقة للعميل
 * ✅ تصفية حسب السعر والخصم
 * ✅ سياسة الاسترجاع - شروط وأحكام - FAQ
 */
...
.free-ship-notice{background:#dcfce7;color:#166534;text-align:center;padding:6px 16px;
  font-size:11px;font-weight:700;border-bottom:1px solid #bbf7d0}
...
/* FILTER SECTION */
.filter-section{background:white;border-radius:16px;padding:12px 16px;margin:0 14px 14px;
  border:1px solid #E8DDD5}
body.dark .filter-section{background:#1e1208;border-color:#3d2a1a}
.filter-range{display:flex;align-items:center;gap:10px;margin-bottom:10px;flex-wrap:wrap}
.filter-range input{flex:1;min-width:100px}
.filter-label{font-size:12px;font-weight:700;color:#7A6A5A;min-width:60px}
...
.pc-img.out-of-stock{opacity:0.5;filter:grayscale(0.2)}
...
.add-b.disabled{opacity:0.5;cursor:not-allowed}
...
/* QUICK ORDER TABLE */
.quick-order-table{width:100%;border-collapse:collapse;margin-bottom:16px}
.quick-order-table th,.quick-order-table td{border:1px solid #E8DDD5;padding:10px;text-align:center}
.quick-order-table th{background:#FF6B35;color:white;font-weight:700}
.quick-order-table input{width:80px;padding:6px;border:1px solid #E8DDD5;border-radius:8px;text-align:center}

/* LOYALTY BANNER */
.loyalty-banner{background:linear-gradient(135deg,#fef9c3,#fde68a);border-radius:12px;
  padding:12px 16px;margin:0 14px 14px;display:flex;justify-content:space-between;
  align-items:center;cursor:pointer}
.loyalty-points{background:#FF6B35;color:white;border-radius:20px;padding:4px 12px;font-weight:800}
...
.orders-table th,.orders-table td{border:1px solid #E8DDD5;padding:8px 10px;text-align:center}
.orders-table th{background:#FF6B35;color:white;font-weight:700}
.orders-table .status-pending{color:#92400E;background:#FEF9C3}
.orders-table .status-processing{color:#1D4ED8;background:#DBEAFE}
.orders-table .status-shipped{color:#5B21B6;background:#E0E7FF}
.orders-table .status-delivered{color:#059669;background:#D1FAE5}
.orders-table .status-cancelled{color:#DC2626;background:#FEE2E2}

/* FAQ */
.faq-item{margin-bottom:12px;border:1px solid #E8DDD5;border-radius:12px;overflow:hidden}
.faq-question{background:#F7F3EF;padding:14px 16px;font-weight:800;cursor:pointer;
  display:flex;justify-content:space-between;align-items:center}
.faq-answer{padding:0 16px;max-height:0;overflow:hidden;transition:max-height 0.3s ease}
.faq-answer.open{max-height:200px;padding:14px 16px}
...
function RegisterModal({ onClose, onSuccess }) {
  ...
  const verify = async () => {
    ...
    const {error}=await supabase.from('customers').insert({
      ...
      address:form.address, password:hashPwd(form.pass), points:0, tier:'M1', total_purchases:0,
      created_at:new Date().toISOString()
    })
    if (error) { showToast('خطأ: '+error.message,true); setLoading(false); return }
    // إنشاء حساب نقاط ولاء للعميل الجديد
    await supabase.from('loyalty_points').insert({
      id:Date.now(), customer_id:Date.now(), points:0, total_earned:0, total_redeemed:0
    }).catch(()=>{})
...
function CartModal({ cart, setCart, onClose, onCheckout, freeShip, currency, promos, customer, loyaltyPoints }) {
  ...
  // خصم نقاط الولاء (كل 10 نقاط = 50 دج)
  const loyaltyDiscountValue = loyaltyPoints ? Math.floor(loyaltyPoints / 10) * 50 : 0
  const [useLoyalty, setUseLoyalty] = useState(false)
  const loyaltyDisc = useLoyalty ? Math.min(loyaltyDiscountValue, cartTotal - buy3Disc - volDisc) : 0
  const pointsToUse = useLoyalty ? Math.min(Math.floor(loyaltyDiscountValue / 50) * 10, loyaltyPoints||0) : 0

  const finalTotal  = cartTotal - buy3Disc - volDisc - loyaltyDisc
            ...
            : <>
              {cart.map(i=>(
                <div key={i.id} className="ci">
                  <div className="ci-img">{i.image?<img src={i.image} style={{width:58,height:58,borderRadius:12,objectFit:'cover'}}/>:<>🛍️</>}</div>
                  <div style={{flex:1}}>
              ...
              {/* نقاط الولاء */}
              {customer && loyaltyPoints > 0 && loyaltyDiscountValue > 0 && (
                <div style={{background:'#fef9c3',borderRadius:14,padding:12,margin:'10px 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap'}}>
                  <div>
                    <span style={{fontWeight:800,color:'#92400E'}}>⭐ لديك {loyaltyPoints} نقطة</span>
                    <span style={{fontSize:12,color:'#92400E',marginRight:8}}>(تساوي {loyaltyDiscountValue} {currency} خصم)</span>
                  </div>
                  <label style={{display:'flex',alignItems:'center',gap:6,fontSize:13,cursor:'pointer'}}>
                    <input type="checkbox" checked={useLoyalty} onChange={e=>setUseLoyalty(e.target.checked)} />
                    استخدم النقاط واحصل على خصم {Math.min(loyaltyDiscountValue, finalTotal + loyaltyDisc).toFixed(0)} {currency}
                  </label>
                </div>
              )}

              {/* الإجمالي */}
              {(buy3Disc>0||volDisc>0||loyaltyDisc>0)&&(
                <div style={{fontSize:13,color:'#94a3b8',textDecoration:'line-through',textAlign:'left',marginBottom:4}}>
                  {cartTotal.toFixed(0)} {currency}
                </div>
              )}
              {loyaltyDisc>0 && (
                <div style={{fontSize:13,color:'#10b981',textAlign:'left',marginBottom:4}}>
                  خصم النقاط: -{loyaltyDisc.toFixed(0)} {currency}
                </div>
              )}
              ...
              <button className="abtn" onClick={()=>onCheckout(finalTotal, buy3Disc+volDisc, loyaltyDisc, pointsToUse)}>
...
function CheckoutModal({ cart, finalTotal, onClose, onSuccess, currency, waNum, storeName, customer, pointsToUse }) {
  ...
  const confirmOrder = async () => {
    ...
    const order = {
      ...
      customer_address: form.address, customer_id: customer?.id || null,
      date: new Date().toLocaleString('ar-DZ'),
      items: JSON.stringify(cart.map(i=>({id:i.id,name:i.name,quantity:i.qty,price:i.price,cartonPrice:i.cartonPrice}))),
      total: finalTotal, status: 'pending',
      loyalty_points_used: pointsToUse || 0
    }
    ...
    // تحديث نقاط الولاء
    if (customer && finalTotal > 0) {
      const loyaltyRate = 100 // كل 100 دج = 1 نقطة
      const earnedPoints = Math.floor(finalTotal / loyaltyRate)
      if (earnedPoints > 0) {
        const {data:lp} = await supabase.from('loyalty_points').select('*').eq('customer_id',customer.id).maybeSingle()
        if (lp) {
          const newPoints = (lp.points || 0) - (pointsToUse || 0) + earnedPoints
          await supabase.from('loyalty_points').update({points: newPoints, total_earned: (lp.total_earned||0) + earnedPoints}).eq('customer_id',customer.id)
        } else {
          await supabase.from('loyalty_points').insert({id:Date.now(), customer_id:customer.id, points: earnedPoints, total_earned: earnedPoints, total_redeemed:0})
        }
        await supabase.from('loyalty_transactions').insert({id:Date.now(), customer_id:customer.id, order_id:orderId, points: earnedPoints, type:'earned', description:'نقاط من الطلب'})
        if (pointsToUse > 0) {
          await supabase.from('loyalty_transactions').insert({id:Date.now()+1, customer_id:customer.id, order_id:orderId, points: -pointsToUse, type:'redeemed', description:'استبدال نقاط بخصم'})
        }
      }
    }

    // إرسال إشعار واتساب لصاحب المتجر
    const adminPhone = localStorage.getItem('nq_admin_wa') || WA_NUM
    const itemsList = cart.map(i => `${i.name} ×${i.qty} = ${(i.price*i.qty).toFixed(0)}`).join('%0A')
    const msg = `🛍️ طلب جديد من متجر نقاء%0A%0A📋 رقم الطلب: ${orderId}%0A👤 العميل: ${form.name}%0A📱 الهاتف: ${form.phone}%0A📍 العنوان: ${form.address}%0A%0A🛒 المنتجات:%0A${itemsList}%0A%0A💰 الإجمالي: ${finalTotal.toFixed(0)} ${currency}%0A%0A✅ لتأكيد الطلب، اضغط على الرابط:%0Ahttps://naqaa-tau.vercel.app/admin?order=${orderId}&confirm=true`
    window.open(`https://wa.me/${adminPhone.replace(/^0/,'213')}?text=${msg}`, '_blank')
    
    showToast('✅ تم إرسال طلبك! سيتم تأكيده عبر واتساب')
    onSuccess(orderId)
    setLoading(false)
  }
    ...
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody">
          ...
          <input className="fi" value={form.name} onChange={F('name')} autoComplete="name" />
          <label className="fi-label">رقم الهاتف *</label>
          <input className="fi" type="tel" value={form.phone} onChange={F('phone')} inputMode="numeric" autoComplete="tel" />
          <label className="fi-label">العنوان</label>
          <textarea className="fi" rows="2" value={form.address} onChange={F('address')} style={{resize:'none'}} />
          ...
          <button className="abtn" onClick={confirmOrder} disabled={loading}>
            {loading?'⏳ جاري...':'✅ تأكيد الطلب وإرسال للمتجر'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet">
        <div className="mhandle"></div>
        <div className={`pc-img ${(p.stock||0)===0 ? 'out-of-stock' : ''}`}>
          ...
          {(p.stock||0)===0 && <span className="badge b-outstock">نفذ</span>}
        </div>
        <div className="mhead">
        ...
        <div className="mbody">
          ...
          <div style={{background:'#F0FDF4',borderRadius:12,padding:12,marginBottom:12}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              <span style={{fontWeight:700}}>📦 سعر القطعة:</span>
              <span>{p.price} {currency}</span>
            </div>
            <div style={{display:'flex',justifyContent:'space-between'}}>
              <span style={{fontWeight:700}}>📦 سعر الكرتون ({p.units||12} قطعة):</span>
              <span style={{color:'#10b981',fontWeight:800}}>{cartonPrice.toFixed(0)} {currency}</span>
            </div>
          </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
              {volTiers.map(({qty,disc})=>(
                <div key={qty} style={{background:'white',borderRadius:10,padding:'7px 4px',textAlign:'center',border:'1px solid #10b981'}}>
                  <div style={{fontWeight:800,fontSize:13}}>{qty}+ كرتون</div>
                  <div style={{color:'#10b981',fontWeight:700,fontSize:12}}>{disc}% خصم</div>
                  <div style={{fontSize:11,color:'#065f46'}}>{(p.price*(1-disc/100)).toFixed(0)} {currency}/قطعة</div>
                </div>
              ))}
            </div>
          </div>

          <button className={`abtn ${(p.stock||0)===0 ? 'disabled' : ''}`} 
            onClick={()=>{if((p.stock||0)!==0){onAddCart(p);onClose()}}} 
              ...
              <div className="hscroll">
                {related.map(r=>(
                  <div key={r.id} onClick={()=>onShowProduct(r)}
                    ...
                    <div className={`pc-img ${(r.stock||0)===0 ? 'out-of-stock' : ''}`}>
    ...
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody">
          ...
          <input className="fi" value={num} onChange={e=>setNum(e.target.value)} placeholder="أدخل رقم طلبك" inputMode="numeric" />
          <button className="abtn" onClick={track}><i className="fas fa-search"></i> تتبع</button>
          ...
          {res&&res.id&&(
            <div style={{marginTop:16}}>
              <div style={{background:'#FFF0EB',borderRadius:14,padding:14,marginBottom:16}}>
                ...
                <div style={{color:'#7A6A5A',fontSize:13,marginTop:4}}>{res.customer_name}</div>
              ...
              {steps.map((s,i)=>{
                ...
                return (
                  <div key={s} className="trstep">
                    ...
                    <div style={{paddingTop:8,fontWeight:700,fontSize:14,color:i<=cur?'#FF6B35':'#7A6A5A'}}>{labels[s]}</div>
                  </div>
                )
              })}
            </div>
...
// صفحة الطلبات السابقة للعميل المسجل
function MyOrdersModal({ customer, onClose, currency }) {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      if (!customer) return
      const { data } = await supabase.from('orders').select('*').eq('customer_id', customer.id).order('id', {ascending: false})
      setOrders(data || [])
      setLoading(false)
    }
    loadOrders()
  ...
  const statusLabel = (status) => {
    switch(status) {
      ...
      default: return status || 'قيد الانتظار'
    }
  }
  ...
  return (
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet">
        ...
        <div className="mbody">
          {loading ? <div className="empty">⏳ جاري التحميل...</div> : orders.length === 0 ? 
            <div className="empty"><i className="fas fa-shopping-bag"></i><p>لا توجد طلبات سابقة</p></div> :
            <div style={{overflowX:'auto'}}>
              <table className="orders-table">
                <thead>
                  <tr><th>رقم الطلب</th><th>التاريخ</th><th>الإجمالي</th><th>الحالة</th><th>التفاصيل</th></tr>
                </thead>
                <tbody>
                  {orders.map(o => {
                    ...
                    return (
                      <tr key={o.id}>
                        <td>{o.id}</td>
                        <td>{o.date}</td>
                        <td style={{color:'#FF6B35',fontWeight:800}}>{Number(o.total).toFixed(0)} {currency}</td>
                        <td><span className={statusClass(o.status)} style={{padding:'3px 8px',borderRadius:20,fontSize:11}}>{statusLabel(o.status)}</span></td>
                        <td>
                          <button className="qty-b" style={{width:'auto',padding:'4px 12px'}} 
                            onClick={() => showToast(items.map(i=>`${i.name} ×${i.quantity} = ${(i.price*i.quantity).toFixed(0)}`).join('\n'))}>
                            📋
                          </button>
                        </td>
                      </tr>
...
// صفحة الطلب السريع (Quick Order)
function QuickOrderModal({ products, onAddToCart, onClose, currency }) {
  const [selectedProducts, setSelectedProducts] = useState([])

  const addProduct = () => {
    setSelectedProducts([...selectedProducts, {id: '', name: '', quantity: 1, price: 0}])
  }
  ...
  const updateProduct = (index, field, value) => {
    ...
    if (field === 'id') {
      ...
      if (prod) {
        newProducts[index].name = prod.name
        newProducts[index].price = prod.price
      }
    }
  ...
  const addAllToCart = () => {
    ...
    showToast(`✅ تم إضافة ${added} منتج إلى السلة`)
    onClose()
  }
        ...
        <div className="mbody">
          <div style={{overflowX:'auto'}}>
            <table className="quick-order-table">
              <thead>
                <tr><th>المنتج</th><th>الكمية (كرتون)</th><th>سعر الوحدة</th><th></th></tr>
              </thead>
              <tbody>
                {selectedProducts.map((item, idx) => (
                  <tr key={idx}>
                    <td>
                      <select value={item.id} onChange={e=>updateProduct(idx, 'id', e.target.value)} style={{width:'100%',padding:6,borderRadius:8,border:'1px solid #E8DDD5'}}>
                        <option value="">اختر منتجاً</option>
                        {products.map(p => (
                          <option key={p.id} value={p.id} disabled={(p.stock||0)===0}>{p.name} {(p.stock||0)===0?'(نفذ)':''}</option>
                        ))}
                      </select>
                    </td>
                    <td><input type="number" min="1" value={item.quantity} onChange={e=>updateProduct(idx, 'quantity', parseInt(e.target.value)||1)} style={{width:70,textAlign:'center'}}/></td>
                    <td>{Number(item.price).toFixed(0)} {currency}</td>
                    <td><button className="qty-b" onClick={()=>removeProduct(idx)} style={{background:'#fee2e2',color:'#dc2626'}}>🗑️</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{display:'flex',gap:10,marginBottom:16}}>
            <button className="abtn" style={{background:'#64748b'}} onClick={addProduct}>➕ إضافة منتج</button>
            <button className="abtn" onClick={addAllToCart}>🛒 إضافة الكل إلى السلة</button>
          </div>
          <p style={{fontSize:12,color:'#64748b',textAlign:'center'}}>💡 يمكنك إضافة عدة منتجات مرة واحدة وكميات مختلفة</p>
        </div>
      </div>
    </div>
  )
...
function ReturnPolicyModal({ onClose, content }) {
  return (
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet center">
        <div className="mhead"><h3>🔄 سياسة الاسترجاع</h3><button className="mclose" onClick={onClose}>×</button></div>
        <div className="mbody">
          <div style={{whiteSpace:'pre-wrap',lineHeight:1.8,color:'#1A0A00'}}>{content || 'يمكن للعميل استرجاع المنتج خلال 14 يوم من الاستلام بشرط أن يكون بحالته الأصلية.\n\nشروط الاسترجاع:\n• المنتج بدون استخدام\n• مع الفاتورة الأصلية\n• خلال 14 يوم'}</div>
        </div>
      </div>
    </div>
  )
...
function FAQModal({ onClose, faqData }) {
  ...
  return (
    <div className="moverlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="msheet center">
        <div className="mhead"><h3>❓ الأسئلة الشائعة</h3><button className="mclose" onClick={onClose}>×</button></div>
        <div className="mbody">
          {faq.length === 0 ? (
            <div className="empty">لا توجد أسئلة شائعة حالياً</div>
          ) : (
            faq.map((item, idx) => (
...
export default function Store() {
  ...
  const [checkoutLoyaltyDisc, setCheckoutLoyaltyDisc] = useState(0)
  const [checkoutPointsUsed, setCheckoutPointsUsed] = useState(0)
  ...
  const RETURN_POLICY = settings['return_policy'] || ''
  const FAQ_DATA = (() => { try { return JSON.parse(settings['faq'] || '[]') } catch { return [] } })()
  ...
  // تحميل نقاط الولاء عند تسجيل الدخول
  useEffect(()=>{
    const loadLoyalty = async () => {
      if (customer && customer.id) {
        const { data } = await supabase.from('loyalty_points').select('points').eq('customer_id', customer.id).maybeSingle()
  ...
  const addToCart=useCallback((p, qty=1)=>{
    ...
    setCart(prev=>{
      ...
      return [...prev,{id:p.id,name:p.name,price:Number(p.price),qty,cartonPrice,image:p.image,units:p.units||12}]
    })
  },[])
  ...
  // تحديد الحد الأقصى للسعر للفلتر
  const maxProductPrice = allP.length > 0 ? Math.max(...allP.map(p => p.price)) : 5000
  ...
  const PC = ({ p }) => {
    ...
    return (
      <div className="pc" onClick={()=>{setDetailProd(p);setModal('detail')}}>
        <div className={`pc-img ${outOfStock ? 'out-of-stock' : ''}`}>
          ...
          {outOfStock && <span className="badge b-outstock">نفذ</span>}
        ...
        <div className="pc-carton">📦 كرتون: {cartonPrice.toFixed(0)} {CUR}</div>
        {(p.stock||0)<10 && (p.stock||0)>0 && <div className="pc-stock">⚠️ {p.stock} كرتون فقط</div>}
        <button className={`add-b ${outOfStock ? 'disabled' : ''}`} disabled={outOfStock}
          onClick={e=>{e.stopPropagation(); if(!outOfStock) addToCart(p)}}>
          <i className="fas fa-cart-plus"></i>
          {outOfStock ? 'نفذ' : 'أضف للسلة'}
        </button>
      </div>
    )
  }
  ...
  const FreeShippingNotice = () => {
    ...
    if (cartTotal >= FREESHIP) {
      return <div className="free-ship-notice">🎉 تهانينا! أنت مؤهل للتوصيل المجاني 🎉</div>
    }
    return <div className="free-ship-notice">🚚 أضف {remaining.toFixed(0)} {CUR} فقط للحصول على توصيل مجاني!</div>
  }
  ...
  const Home = () => (
    <>
      ...
      {customer && loyaltyPoints > 0 && (
        <div className="loyalty-banner" onClick={()=>setModal('loyalty')}>
          <div>⭐ <strong>نقاط الولاء</strong></div>
          <div className="loyalty-points">{loyaltyPoints} نقطة</div>
        </div>
      )}
      ...
      <div className="promo-strip">
        <div className="promo-box" style={{background:'linear-gradient(135deg,#10b981,#059669)'}} onClick={()=>{setSortSel('promo_first');setTab('search')}}>
          <div style={{fontSize:24}}>🎁</div><div style={{color:'white',fontWeight:800,fontSize:13,marginTop:4}}>العروض أولاً</div>
          <div style={{color:'rgba(255,255,255,.8)',fontSize:11}}>المنتجات المخفضة تظهر أولاً</div>
        </div>
        <div className="promo-box" style={{background:'linear-gradient(135deg,#3b82f6,#1d4ed8)'}} onClick={()=>{setSortSel('price_asc');setTab('search')}}>
          <div style={{fontSize:24}}>📦</div><div style={{color:'white',fontWeight:800,fontSize:13,marginTop:4}}>خصم الكميات</div>
  ...
  const SearchTab = () => (
    <div className="sec" style={{marginTop:14}}>
      ...
      <button className="sec-more" style={{marginBottom:10}} onClick={()=>setShowFilters(!showFilters)}>
      ...
      {showFilters && (
        <div className="filter-section">
          <div className="filter-range">
            <span className="filter-label">💰 السعر:</span>
            <input type="range" min="0" max={maxProductPrice} value={minPrice} onChange={e=>{setMinPrice(parseInt(e.target.value)); setPage(1)}} />
            <input type="range" min="0" max={maxProductPrice} value={maxPrice} onChange={e=>{setMaxPrice(parseInt(e.target.value)); setPage(1)}} />
            <span>{minPrice} - {maxPrice} {CUR}</span>
          </div>
          <div className="filter-range">
            <span className="filter-label">🎯 خصم أكبر من:</span>
            <select value={minDiscount} onChange={e=>{setMinDiscount(parseInt(e.target.value)); setPage(1)}} style={{...S.input,width:'auto',padding:'6px 12px'}}>
      ...
      <div className="chips" style={{marginBottom:14}}>
        {[['promo_first','🎯 العروض أولاً'],['newest','الأحدث'],['price_asc','السعر ↑'],['price_desc','السعر ↓']].map(([v,l])=>(
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}}>
            {categories.map(c=>(
              <div key={c.id} onClick={()=>{setCatSel(c.id);setTab('search')}} style={{background:'white',borderRadius:16,padding:14,display:'flex',alignItems:'center',gap:12,cursor:'pointer',boxShadow:'0 2px 10px rgba(0,0,0,.07)',transition:'.2s',overflow:'hidden'}}>
                {c.image ? <img src={c.image} style={{width:50,height:38,borderRadius:10,objectFit:'cover',flexShrink:0}}/> : <div style={{width:50,height:38,borderRadius:10,background:'#FFF0EB',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22}}>📦</div>}
  ...
  return (
    <div dir="rtl">
      ...
      <div className="sh">
        <div className="sh-top">
          ...
          <div className="sh-right">
            ...
            {customer ? (
              <button className="sh-login" onClick={()=>setModal('account')}><i className="fas fa-user"></i> {customer.name.split(' ')[0]}</button>
            ) : (
      ...
      <div className={`drawer${drawerOpen?' open':''}`}>
        ...
        <div className="drawer-nav">
          {[
            ...
            {id:'quickOrder', e:'⚡', l:'طلب سريع', a:()=>setModal('quickOrder')},
            {id:'cart-d', e:'🛒', l:'السلة', b:cartCount, a:()=>setModal('cart')},
            {id:'track', e:'📍', l:'تتبع الطلب', a:()=>setModal('tracking')},
            null,
            {id:'myOrders', e:'📋', l:'طلباتي السابقة', a:()=>setModal('myOrders'), show:!!customer},
            {id:'returnPolicy', e:'🔄', l:'سياسة الاسترجاع', a:()=>setModal('returnPolicy')},
            {id:'faq', e:'❓', l:'الأسئلة الشائعة', a:()=>setModal('faq')},
            {id:'terms', e:'📜', l:'شروط وأحكام', a:()=>setModal('terms')},
            null,
            {id:'auth', e:'👤', l:customer?customer.name:'تسجيل الدخول', a:()=>setModal(customer?'account':'login'), show:!customer},
            {id:'logout', e:'🚪', l:'تسجيل خروج', a:()=>{setCustomer(null); localStorage.removeItem('nq_customer'); showToast('تم الخروج'); setDrawerOpen(false)}, show:!!customer},
          ...
          ].filter(it => it === null || (it.show !== false)).map((it,i)=>it===null ? <div key={i} className="di-div"/> : (
            <div key={it.id} className={`di${tab===it.id&&!it.a?' act':''}`} onClick={()=>{(it.a?it.a():setTab(it.id)); setDrawerOpen(false)}}>
              <div className="di-ico">{it.e}</div><span style={{flex:1}}>{it.l}</span>
      ...
      <div className="wa-float">
        <button className="wa-btn" onClick={()=>window.open(`https://wa.me/${WA}`,'_blank')}><i className="fab fa-whatsapp" style={{fontSize:28,color:'white'}}></i></button>
      ...
      {modal==='cart' && <CartModal cart={cart} setCart={setCart} onClose={()=>setModal(null)} onCheckout={(total,disc,loyaltyDisc,pointsUsed)=>{setCheckoutTotal(total); setCheckoutLoyaltyDisc(loyaltyDisc); setCheckoutPointsUsed(pointsUsed); setModal('checkout')}} freeShip={FREESHIP} currency={CUR} promos={promos} customer={customer} loyaltyPoints={loyaltyPoints}/>}
      {modal==='checkout' && <CheckoutModal cart={cart} finalTotal={checkoutTotal} onClose={()=>setModal('cart')} onSuccess={id=>{setCart([]); setThankId(id); setModal('thankyou')}} currency={CUR} waNum={WA} storeName={SNAME} customer={customer} pointsToUse={checkoutPointsUsed}/>}
      ...
      {modal==='myOrders' && <MyOrdersModal customer={customer} onClose={()=>setModal(null)} currency={CUR}/>}
      {modal==='quickOrder' && <QuickOrderModal products={products} onAddToCart={addToCart} onClose={()=>setModal(null)} currency={CUR}/>}
      {modal==='returnPolicy' && <ReturnPolicyModal onClose={()=>setModal(null)} content={RETURN_POLICY}/>}
      {modal==='faq' && <FAQModal onClose={()=>setModal(null)} faqData={FAQ_DATA}/>}
      {modal==='terms' && <ReturnPolicyModal onClose={()=>setModal(null)} content={TERMS} title="شروط وأحكام"/>}
    </div>
  )
}
