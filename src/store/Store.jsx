/**
 * Store.jsx — نقاء v7 (مصحح بالكامل)
...
.pc-promo-badge{position:absolute;top:6px;right:6px;padding:3px 8px;border-radius:20px;
  font-size:10px;font-weight:800;color:white;background:#FF6B35;z-index:2}
...
.otp-input{width:52px;height:58px;border:2px solid #E8DDD5;border-radius:12px;
  ...
  outline:none;background:#F7F3EF}
    ...
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody" style={{ textAlign: 'center' }}>
          ...
          <button style={{ background: 'none', border: 'none', color: '#FF6B35', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }} onClick={() => { setStep(1); setDigits(['', '', '', '']) }}>← تعديل البيانات</button>
        </div>
      </div>
    </div>
  )
...
function CartModal({ cart, setCart, onClose, onCheckout, freeShip, currency, promos }) {
  ...
  const volTiers = [{ min: 500, disc: 5 }, { min: 1000, disc: 10 }, { min: 2000, disc: 15 }]
  ...
  return (
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet">
        ...
        <div className="mhead"><h3>🛒 سلة المشتريات ({cart.reduce((s, i) => s + i.qty, 0)} كرتون)</h3><button className="mclose" onClick={onClose}>×</button></div>
        <div className="mbody">
          {cart.length === 0 ? <div className="empty"><i className="fas fa-shopping-cart"></i><p>السلة فارغة</p></div> : <>
            {cart.map(i => (
              <div key={i.id} className="ci">
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: '#FF6B35', fontWeight: 900, fontSize: 15, marginTop: 2 }}>{i.price} {currency} × {i.qty} = {(i.price * i.qty).toFixed(0)} {currency}</div>
            ...
            {cartTotal < freeShip && (
              <div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: 10, padding: '10px 12px', marginBottom: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#C2410C', marginBottom: 5 }}>🚚 أضف <strong style={{ color: '#FF6B35', fontSize: 15 }}>{(freeShip - cartTotal).toFixed(0)} {currency}</strong> للتوصيل المجاني!</div>
            ...
            {cartTotal >= freeShip && <div style={{ background: '#D1FAE5', borderRadius: 10, padding: '8px 12px', marginBottom: 12, textAlign: 'center', fontWeight: 700, color: '#059669', fontSize: 13 }}>🎉 أحسنت! التوصيل مجاني لهذا الطلب</div>}
            <div className="prog-bar-wrap">
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 700 }}>
                {currentTier ? <span style={{ color: '#10b981' }}>🎉 خصم {currentTier.disc}% مفعّل! وفّرت {volDisc.toFixed(0)} {currency}</span> : <span>أضف {volTiers.find(t => cartTotal < t.min)?.min - cartTotal || 0} {currency} للحصول على خصم</span>}
              </div>
              <div className="prog-track"><div className="prog-fill" style={{ width: `${Math.min(100, cartTotal / 2000 * 100)}%` }} /></div>
            </div>
            {(buy3Disc > 0 || volDisc > 0) && <div style={{ fontSize: 13, color: '#94a3b8', textDecoration: 'line-through', textAlign: 'left', marginBottom: 4 }}>{cartTotal.toFixed(0)} {currency}</div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 900, fontSize: 18, marginBottom: 16 }}><span>الإجمالي</span><span style={{ color: '#FF6B35' }}>{finalTotal.toFixed(0)} {currency}</span></div>
            <button className="abtn" onClick={() => onCheckout(finalTotal, buy3Disc + volDisc)}><i className="fas fa-credit-card"></i> إتمام الشراء</button>
          </>}
        </div>
      </div>
    </div>
...
function CheckoutModal({ cart, finalTotal, onClose, onSuccess, currency, waNum, storeName }) {
  ...
  const confirmOrder = async () => {
    ...
    setLoading(true)
    const order = {
      id: Date.now(), customer_name: form.name, phone: form.phone,
      address: form.address, date: new Date().toLocaleString('ar-DZ'),
    ...
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody" style={{ textAlign: 'center' }}>
          ...
          <div style={{ background: '#fef9c3', borderRadius: 12, padding: 12, marginBottom: 16, fontSize: 13 }}>🔑 كودك: <strong style={{ fontSize: 20, color: '#dc2626' }}>{genOtp}</strong></div>
          <button className="abtn green" onClick={confirmOrder} disabled={loading || otp.length < 4}>{loading ? '⏳...' : '✅ تأكيد الطلبية'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody">
          ...
          <input className="fi" type="tel" value={form.phone} onChange={F('phone')} inputMode="numeric" autoComplete="tel" onKeyPress={e => { if (!/[0-9+]/.test(e.key)) e.preventDefault() }} />
          <label className="fi-label">العنوان</label>
          <textarea className="fi" rows="2" value={form.address} onChange={F('address')} style={{ resize: 'none' }} autoComplete="street-address"></textarea>
...
function DetailModal({ product, wishlist, onClose, onAddCart, onToggleWish, currency, products, onShowProduct, promos }) {
  ...
  return (
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet">
        ...
        <div className="mhead"><h3 style={{ flex: 1, fontSize: 15 }}>{p.name}</h3><button className="mclose" onClick={onClose}>×</button></div>
        <div className="mbody">
          ...
          <button className="abtn" onClick={() => { onAddCart(p); onClose() }} disabled={(p.stock || 0) === 0}>{(p.stock || 0) === 0 ? 'نفذ من المخزون' : 'أضف للسلة'}</button>
          {related.length > 0 && (
      ...
      {reviews.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16, background: '#FFF7ED', borderRadius: 12, padding: 14 }}>
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 1, margin: '4px 0' }}>{[1, 2, 3, 4, 5].map(n => <span key={n} style={{ color: n <= Math.round(avgR) ? '#FF6B35' : '#E2E8F0', fontSize: 14 }}>★</span>)}</div>
      ...
      {reviews.map(r => (
        <div key={r.id} style={{ borderBottom: '1px solid #F1ECE8', padding: '12px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 5 }}>
            <div><strong style={{ fontSize: 13, color: '#1A0A00' }}>{r.customer_name}</strong><div style={{ display: 'flex', gap: 1, marginTop: 2 }}>{[1, 2, 3, 4, 5].map(n => <span key={n} style={{ color: n <= r.rating ? '#FF6B35' : '#E2E8F0', fontSize: 13 }}>★</span>)}</div></div>
...
function TrackingModal({ onClose, currency }) {
  ...
  const statusColors = { pending: '#f59e0b', processing: '#3b82f6', shipped: '#7c3aed', delivered: '#10b981' }
      ...
      <div className="msheet center">
        ...
        <div className="mbody">
          ...
          {res === false && orders.length === 0 && !loading && (num || phone) && (
            <div style={{ textAlign: 'center', padding: '20px 0', color: '#ef4444' }}><div style={{ fontSize: 32, marginBottom: 8 }}>❌</div><p style={{ fontWeight: 700 }}>لا توجد طلبيات بهذه البيانات</p></div>
          )}
          {res && res.id && (
  ...
  return (
    <div className="moverlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="msheet center">
        ...
        <div className="mbody">
          <div style={{ textAlign: 'center', marginBottom: 20 }}><div style={{ fontSize: 40 }}>🛍️</div><div style={{ fontWeight: 900, fontSize: 18, marginTop: 8 }}>{settings['store_name'] || 'نقاء'}</div></div>
...
function PromoCountdown({ endDate }) {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setT({ h: String(Math.floor(diff / 3600000)).padStart(2, '0'), m: String(Math.floor(diff % 3600000 / 60000)).padStart(2, '0'), s: String(Math.floor(diff % 60000 / 1000)).padStart(2, '0') })
    }
    tick();
    const id = setInterval(tick, 1000);
    return () => clearInterval(id)
  }, [endDate])
  return <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 700 }}>⏳</span>{[t.h, t.m, t.s].map((v, i) => (<span key={i} style={{ display: 'flex', alignItems: 'center', gap: 2 }}><span style={{ background: '#1A0A00', color: 'white', padding: '3px 6px', borderRadius: 6, fontSize: 13, fontWeight: 900, fontFamily: 'monospace' }}>{v}</span>{i < 2 && <span style={{ color: '#94a3b8', fontWeight: 900 }}>:</span>}</span>))}</div>
}
...
export default function Store() {
  ...
  const addToCart = useCallback((p, qty = 1) => {
    ...
    setCart(prev => {
      ...
      if (existing) { showToast('✅ تمت زيادة الكمية'); return prev.map(i => i.id === p.id ? { ...i, qty: i.qty + qty } : i) }
  ...
  const PC = ({ p }) => {
    ...
    return (
      <div className="pc" onClick={() => { setDetailProd(p); setModal('detail') }}>
        <div className="pc-img" style={{ opacity: (p.stock || 0) === 0 ? 0.45 : 1 }}>
          ...
          <button className="fav-b" onClick={e => { e.stopPropagation(); toggleWish(p.id) }}><i className="fas fa-heart" style={{ color: isW ? '#FF6B35' : '#CBD5E1' }}></i></button>
        </div>
        <div className="pc-name">{p.name}</div>
        {hasDisc ? <div><div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2, flexWrap: 'wrap' }}><span style={{ background: '#888', color: 'white', fontSize: 11, fontWeight: 900, padding: '2px 7px', borderRadius: 20 }}>{pct}%</span><span style={{ fontSize: 12, color: '#94a3b8', textDecoration: 'line-through', fontWeight: 600 }}>{p.price}{CUR}</span></div><div style={{ fontSize: 16, fontWeight: 900, color: '#1A0A00' }}>{fp}{CUR}</div></div> : <div style={{ fontSize: 16, fontWeight: 900, color: '#FF6B35' }}>{fp} {CUR}</div>}
        ...
        <div style={{ display: 'flex', gap: 5, marginTop: 'auto', paddingTop: 6 }}>
          <button className="add-b" style={{ flex: 1 }} disabled={(p.stock || 0) === 0} onClick={e => { e.stopPropagation(); addToCart(p) }}><i className="fas fa-cart-plus"></i>{(p.stock || 0) === 0 ? 'نفذ' : 'أضف'}</button>
          <button onClick={e => { e.stopPropagation(); const msg = `🛍️ ${p.name}%0A💰 السعر: ${p.price} ${CUR}%0A🔗 ${window.location.origin}`; window.open(`https://wa.me/?text=${msg}`, '_blank') }} style={{ width: 32, height: 32, borderRadius: 8, background: '#25D36610', color: '#25D366', border: '1px solid #25D36630', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><i className="fab fa-whatsapp" style={{ fontSize: 14 }}></i></button>
        </div>
      </div>
    )
  }
  ...
  const Home = () => {
    ...
    const bestSellersProds = bestSellers.map(id => allP.find(p => String(p.id) === id)).filter(Boolean).slice(0, 12)
      ...
      <>
        ...
        <div className="banner-wrap">
          <div className="banner-track" style={{ transform: `translateX(${bannerIdx * 100}%)` }}>
            {banners.length > 0 ? banners.map((b, i) => (b.image ? <img key={i} src={b.image} className="banner-slide" alt="" /> :
        ...
        {activePromoBanner && (
          <div className="flash-bar" onClick={() => setTab('search')}>
            <div><div style={{ color: 'white', fontWeight: 900, fontSize: 16 }}>⚡ {activePromoBanner.name}</div><div style={{ color: 'rgba(255,255,255,.8)', fontSize: 12 }}>{activePromoBanner.description || 'عرض لفترة محدودة'}</div></div>
        ...
        {brands.length > 0 && (
          <div className="sec"><div className="sec-head"><span className="sec-title">⭐ أفضل الماركات</span><button className="sec-more" onClick={() => setDrawerOpen(true)}>عرض الكل</button></div>
            <div className="anim-grid"><div className="anim-all" onClick={() => { setBrandSel('all'); setTab('search') }}><i className="fas fa-th"></i><span>عرض الكل</span></div>
              {brands.slice(0, 5).map(b => (<div key={b.id} className={`anim-card${brandSel == b.id ? ' sel' : ''}`} onClick={() => { setBrandSel(b.id); setTab('search') }}>{b.image ? <><img src={b.image} alt={b.name} /><div className="overlay"><span>{b.name}</span></div></> : <div className="no-img">{b.name}</div>}</div>))}
            </div>
          </div>
        )}
        {categories.length > 0 && (
          <div className="sec"><div className="sec-head"><span className="sec-title">📂 الفئات</span><button className="sec-more" onClick={() => setTab('cats')}>عرض الكل</button></div>
            <div className="cats-scroll">{categories.map(c => (<div key={c.id} className={`cat-item${catSel == c.id ? ' sel' : ''}`} onClick={() => { setCatSel(c.id); setTab('search') }}><div className="cat-img">{c.image ? <img src={c.image} alt={c.name} /> : <span>📁</span>}</div><div className="cat-label">{c.name}</div></div>))}</div>
          </div>
        )}
        <div className="promo-strip">
          <div className="promo-box" style={{ background: 'linear-gradient(135deg,#10b981,#059669)' }} onClick={() => { setSortSel('newest'); setTab('search') }}><div style={{ fontSize: 24 }}>🎁</div><div style={{ color: 'white', fontWeight: 800, fontSize: 13, marginTop: 4 }}>اشتري 3 خذ 4</div><div style={{ color: 'rgba(255,255,255,.8)', fontSize: 11 }}>أرخص منتج مجاناً</div></div>
          <div className="promo-box" style={{ background: 'linear-gradient(135deg,#3b82f6,#1d4ed8)' }} onClick={() => { setSortSel('price_asc'); setTab('search') }}><div style={{ fontSize: 24 }}>📦</div><div style={{ color: 'white', fontWeight: 800, fontSize: 13, marginTop: 4 }}>خصم الكميات</div><div style={{ color: 'rgba(255,255,255,.8)', fontSize: 11 }}>500دج→5% | 1000دج→10%</div></div>
        </div>
        {dayDeal && (
          <div className="day-deal"><div style={{ background: 'linear-gradient(135deg,#FF6B35,#E8430E)', padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: 'white', fontWeight: 900, fontSize: 15 }}>🌟 عرض اليوم</span><span style={{ color: 'white', fontSize: 13 }}>خصم {dayDeal.discount}%</span></div>
            <div style={{ display: 'flex', gap: 16, padding: 16, cursor: 'pointer' }} onClick={() => { setDetailProd(dayDeal); setModal('detail') }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{dayDeal.name}</div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 12, color: '#94a3b8', textDecoration: 'line-through' }}>{dayDeal.price} {CUR}</span><span style={{ fontSize: 20, fontWeight: 900, color: '#FF6B35' }}>{(dayDeal.price * (1 - dayDeal.discount / 100)).toFixed(0)} {CUR}</span></div><button className="add-b" style={{ marginTop: 8 }} onClick={e => { e.stopPropagation(); addToCart(dayDeal) }}>أضف للسلة</button></div>
            </div>
          </div>
        )}
        {flashP.length > 0 && (<div className="sec"><div className="sec-head"><span className="sec-title">⚡ عروض خاصة</span><button className="sec-more" onClick={() => setTab('search')}>عرض الكل</button></div><div className="hscroll">{flashP.map(p => <PC key={p.id} p={p} />)}</div></div>)}
        {newP.length > 0 && (<div className="sec"><div className="sec-head"><span className="sec-title">🎁 وصل حديثاً</span></div><div className="hscroll">{newP.slice(0, 10).map(p => <PC key={p.id} p={p} />)}</div></div>)}
        {bestSellersProds.length > 0 && (<div className="sec"><div className="sec-head"><span className="sec-title">🔥 الأكثر مبيعاً</span><button className="sec-more" onClick={() => setTab('search')}>عرض الكل</button></div><div className="hscroll">{bestSellersProds.map(p => <PC key={p.id} p={p} />)}</div></div>)}
        {allP.length > 0 && (<div className="sec"><div className="sec-head"><span className="sec-title">📦 جميع المنتجات</span><button className="sec-more" onClick={() => setTab('search')}>عرض الكل</button></div><div className="hscroll">{allP.slice(0, 10).map(p => <PC key={p.id} p={p} />)}</div></div>)}
        {cartCount > 0 && (<div className="cart-bar" onClick={() => setModal('cart')}><span style={{ color: 'white', fontWeight: 700, fontSize: 14 }}>🛒 {cartCount} كرتون في السلة</span><span style={{ color: 'white', fontWeight: 900, fontSize: 16 }}>{cartTotal.toFixed(0)} {CUR}</span></div>)}
        <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 13, padding: '32px 0 8px', borderTop: '1px solid #e2e8f0', margin: '20px 14px 0' }}>© 2025 {SNAME} — جميع الحقوق محفوظة</div>
      </>
    )
  }

  const SearchTab = () => {
    const absoluteMax = Math.max(...allP.map(p => Number(p.price) || 0), 1000)
    return (
      <div className="sec" style={{ marginTop: 14 }}>
        ...
        <div className="chips" style={{ marginBottom: 12 }}>{[['newest', 'الأحدث'], ['price_asc', 'السعر ↑'], ['price_desc', 'السعر ↓'], ['disc', 'الخصومات أولاً']].map(([v, l]) => (<button key={v} className={`chip${sortSel === v ? ' sel' : ''}`} onClick={() => { setSortSel(v); setPage(1) }}>{l}</button>))}</div>
        <div style={{ background: 'white', borderRadius: 14, padding: '12px 14px', marginBottom: 12, boxShadow: '0 1px 6px rgba(0,0,0,.06)', border: '1.5px solid #F1ECE8' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 13, fontWeight: 700 }}><span style={{ color: '#1A0A00' }}>🎚️ فلتر السعر</span><span style={{ color: '#FF6B35', fontWeight: 800 }}>{priceMin.toLocaleString()} — {priceMax >= 999999 ? '∞' : priceMax.toLocaleString()} {CUR}</span></div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 4 }}><span style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }}>من</span><input type="range" min={0} max={absoluteMax} step={50} value={Math.min(priceMin, absoluteMax)} onChange={e => setPriceMin(Math.min(Number(e.target.value), priceMax - 50))} style={{ flex: 1, accentColor: '#FF6B35', cursor: 'pointer' }} /><span style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }}>إلى</span><input type="range" min={0} max={absoluteMax} step={50} value={Math.min(priceMax, absoluteMax)} onChange={e => setPriceMax(Math.max(Number(e.target.value), priceMin + 50))} style={{ flex: 1, accentColor: '#FF6B35', cursor: 'pointer' }} /></div>
          {(priceMin > 0 || priceMax < 999999) && <button onClick={() => { setPriceMin(0); setPriceMax(999999) }} style={{ fontSize: 11, color: '#FF6B35', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, fontFamily: 'inherit' }}>✕ إلغاء فلتر السعر</button>}
        </div>
  ...
  const CatsTab = () => {
    return (
      <div className="sec" style={{ marginTop: 14 }}>
        ...
        <div className="anim-grid"><div className="anim-all" onClick={() => { setBrandSel('all'); setCatSel('all'); setTab('search') }}><i className="fas fa-th"></i><span>كل المنتجات</span></div>{brands.map(b => (<div key={b.id} className="anim-card" onClick={() => { setBrandSel(b.id); setTab('search') }}>{b.image ? <><img src={b.image} alt={b.name} /><div className="overlay"><span>{b.name}</span></div></> : <div className="no-img">{b.name}</div>}</div>))}</div>
        {categories.length > 0 && (<div style={{ marginTop: 20 }}><div className="sec-head" style={{ paddingTop: 0 }}><span className="sec-title">📂 الفئات</span></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>{categories.map(c => (<div key={c.id} onClick={() => { setCatSel(c.id); setTab('search') }} style={{ background: 'white', borderRadius: 16, padding: 14, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', boxShadow: '0 2px 10px rgba(0,0,0,.07)' }}>{c.image ? <img src={c.image} style={{ width: 50, height: 38, borderRadius: 10, objectFit: 'cover', flexShrink: 0 }} /> : <div style={{ width: 50, height: 38, borderRadius: 10, background: '#FFF0EB', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>📦</div>}<span style={{ fontWeight: 700, fontSize: 14, color: '#1A0A00' }}>{c.name}</span></div>))}</div></div>)}
      </div>
    )
  }
  ...
  const WishTab = () => {
    ...
    return (<div className="sec" style={{ marginTop: 14 }}>{wp.length === 0 ? <div className="empty"><i className="fas fa-heart"></i><p>قائمة المفضلة فارغة — أضف منتجات من ❤️</p></div> : <div className="prod-grid">{wp.map(p => <PC key={p.id} p={p} />)}</div>}</div>)
  }
    ...
    return (
      <div style={{ paddingBottom: 80 }}>
        <div style={{ background: 'linear-gradient(135deg,#FF6B35,#7C3AED)', padding: '22px 18px 20px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}><div style={{ fontSize: 12, color: 'rgba(255,255,255,.75)', fontWeight: 700, marginBottom: 3 }}>العروض الحصرية</div><h2 style={{ color: 'white', fontWeight: 900, fontSize: 22, marginBottom: 4 }}>🎯 {active.length} عرض نشط</h2><p style={{ color: 'rgba(255,255,255,.8)', fontSize: 13 }}>وفّر أكثر مع عروضنا المتجددة</p></div>
        </div>
        {active.length === 0 && <div className="empty" style={{ marginTop: 40 }}><i className="fas fa-tag" /><p>لا توجد عروض حالياً</p></div>}
        {active.map(promo => {
          const pids = (() => { try { return typeof promo.product_ids === 'string' ? JSON.parse(promo.product_ids || '[]') : (promo.product_ids || []) } catch { return [] } })()
          ...
          return (
            <div key={promo.id} style={{ background: 'white', borderRadius: 20, margin: '12px 14px', boxShadow: '0 4px 20px rgba(0,0,0,.08)', overflow: 'hidden', border: '1.5px solid #F1ECE8' }}>
              ...
              <div style={{ padding: '16px 16px 10px' }}>
                ...
                {(promo.type === 'percent' || promo.type === 'fixed') && (<div style={{ background: 'linear-gradient(135deg,#FF6B35,#E8430E)', color: 'white', borderRadius: 50, padding: '5px 14px', fontSize: 17, fontWeight: 900, display: 'inline-block', marginBottom: 8, float: 'left', boxShadow: '0 4px 12px rgba(255,107,53,.35)' }}>{promo.type === 'percent' ? `-${promo.discount_value}%` : `-${promo.discount_value} ${CUR}`}</div>)}
                ...
                {promo.type === 'tier_buy' && <p style={{ color: '#3b82f6', fontSize: 12, fontWeight: 700, marginBottom: 10 }}>📦 اشتري {promo.tier_qty} كرتون من نفس الشركة → خصم {promo.tier_value}{promo.tier_type === 'percent' ? '%' : ' ' + CUR}</p>}
              </div>
              {promoProds.length > 0 && (<div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 14px 14px' }}>{promoProds.slice(0, 6).map(pp => (<div key={pp.id} style={{ flexShrink: 0, width: 64, textAlign: 'center', cursor: 'pointer' }} onClick={() => { setDetailProd(pp); setModal('detail') }}>{pp.image ? <img src={pp.image} style={{ width: 60, height: 60, borderRadius: 12, objectFit: 'cover', display: 'block', margin: '0 auto 4px', border: '2px solid #F1ECE8' }} /> : <div style={{ width: 60, height: 60, borderRadius: 12, background: '#F8F4F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, margin: '0 auto 4px' }}>🛍️</div>}<div style={{ fontSize: 10, fontWeight: 700, color: '#7A6A5A', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{pp.name}</div></div>))}</div>)}
              <div style={{ padding: '10px 16px 14px', borderTop: '1px solid #F1ECE8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>{promo.end_date ? <PromoCountdown endDate={promo.end_date} /> : <span style={{ fontSize: 12, color: '#94a3b8', fontWeight: 700 }}>⚡ بدون تاريخ انتهاء</span>}<button style={{ background: 'linear-gradient(135deg,#FF6B35,#E8430E)', color: 'white', border: 'none', borderRadius: 30, padding: '8px 18px', fontWeight: 800, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }} onClick={() => setTab('search')}>تسوّق الآن</button></div>
            </div>
          )
        })}
      </div>
  ...
  const QuickOrderTab = () => {
    ...
    const addAll = () => {
      let count = 0
      visibleProds.forEach(p => { const qty = parseInt(qtyMap[p.id]) || 0; if (qty > 0) { addToCart(p, qty); count++ } })
    ...
    return (
      <div className="sec" style={{ marginTop: 14, paddingBottom: 80 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}><h2 className="sec-title">⚡ الطلب السريع</h2><button onClick={addAll} style={{ background: 'linear-gradient(135deg,#FF6B35,#E8430E)', color: 'white', border: 'none', borderRadius: 30, padding: '10px 20px', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}><i className="fas fa-cart-plus"></i> إضافة الكل للسلة</button></div>
        <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.07)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ background: '#FF6B35' }}><th style={{ padding: '11px 12px', textAlign: 'right', color: 'white', fontWeight: 700, fontSize: 13, border: '1px solid rgba(255,255,255,.2)' }}>المنتج</th><th style={{ padding: '11px 12px', textAlign: 'center', color: 'white', fontWeight: 700, fontSize: 13, border: '1px solid rgba(255,255,255,.2)' }}>سعر/كرتون</th><th style={{ padding: '11px 12px', textAlign: 'center', color: 'white', fontWeight: 700, fontSize: 13, border: '1px solid rgba(255,255,255,.2)' }}>الكمية (كرتون)</th><th style={{ padding: '11px 12px', textAlign: 'center', color: 'white', fontWeight: 700, fontSize: 13, border: '1px solid rgba(255,255,255,.2)' }}>الإجمالي</th></tr></thead>
            <tbody>{visibleProds.map((p, i) => {
              const qty = parseInt(qtyMap[p.id]) || 0
              const unitPrice = p.carton_price || p.price * (p.units || 12)
              return (<tr key={p.id} style={{ background: i % 2 === 0 ? 'white' : '#F8FAFC', borderBottom: '1px solid #E2E8F0' }}><td style={{ padding: '10px 12px', border: '1px solid #E2E8F0' }}><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>{p.image && <img src={p.image} style={{ width: 36, height: 36, borderRadius: 8, objectFit: 'cover', border: '1px solid #E2E8F0' }} />}<div><div style={{ fontWeight: 700, fontSize: 13, color: '#1A0A00' }}>{p.name}</div><div style={{ fontSize: 11, color: '#94a3b8' }}>{p.units || 12} قطعة/كرتون</div></div></div></td><td style={{ padding: '10px 12px', textAlign: 'center', fontWeight: 700, color: '#FF6B35', border: '1px solid #E2E8F0' }}>{unitPrice.toFixed(0)} {CUR}</td><td style={{ padding: '10px 12px', textAlign: 'center', border: '1px solid #E2E8F0' }}><input type="number" min="0" value={qtyMap[p.id] || ''} onChange={e => setQtyMap(m => ({ ...m, [p.id]: e.target.value }))} placeholder="0" style={{ width: 70, textAlign: 'center', border: '2px solid #E2E8F0', borderRadius: 8, padding: '5px 8px', fontSize: 14, fontWeight: 700, fontFamily: 'inherit', outline: 'none' }} onFocus={e => e.target.style.borderColor = '#FF6B35'} onBlur={e => e.target.style.borderColor = '#E2E8F0'} /></td><td style={{ padding: '10px 12px', textAlign: 'center', fontWeight: 700, color: '#1A0A00', border: '1px solid #E2E8F0' }}>{qty > 0 ? `${(qty * unitPrice).toFixed(0)} ${CUR}` : '—'}</td></tr>)
            })}</tbody>
            <tfoot><tr style={{ background: '#FFF7ED' }}><td colSpan={3} style={{ padding: '12px', fontWeight: 800, color: '#1A0A00', fontSize: 14, border: '1px solid #E2E8F0', textAlign: 'right' }}>💰 الإجمالي المحدد:</td><td style={{ padding: '12px', fontWeight: 900, color: '#FF6B35', fontSize: 16, border: '1px solid #E2E8F0', textAlign: 'center' }}>{Object.entries(qtyMap).reduce((sum, [id, qty]) => { const p = allP.find(x => String(x.id) === String(id)); const up = p ? (p.carton_price || p.price * (p.units || 12)) : 0; return sum + (parseInt(qty) || 0) * up }, 0).toFixed(0)} {CUR}</td></tr></tfoot>
          </table>
        </div>
      </div>
    )
  ...
  return (
    <div dir="rtl">
      <div className="sh">
        <div className="sh-top"><button className="sh-icon" onClick={() => setDrawerOpen(true)}><i className="fas fa-bars"></i></button><span className="sh-logo">{SNAME}</span><div className="sh-right"><button className="sh-contact" onClick={() => setModal('contact')}><i className="fas fa-phone"></i> اتصل</button>{customer ? <button className="sh-login" onClick={() => setModal('account')}><i className="fas fa-user"></i> {customer.name.split(' ')[0]}</button> : <button className="sh-login" onClick={() => setModal('login')}><i className="fas fa-user"></i> دخول</button>}</div></div>
        <div className="sh-search"><i className="fas fa-search" style={{ color: '#aaa' }}></i><input value={search} onChange={e => { setSearch(e.target.value); setTab('search'); setPage(1) }} placeholder="بحث عن المنتجات..." />{search && <button onClick={() => { setSearch(''); setTab('home') }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 16 }}>×</button>}</div>
      </div>
      {drawerOpen && <div className="drawer-overlay" onClick={() => setDrawerOpen(false)} />}
      <div className={`drawer${drawerOpen ? ' open' : ''}`}>
        <div className="drawer-head"><div style={{ fontSize: 20, fontWeight: 900, color: 'white', marginBottom: 4 }}>🛍️ {SNAME}</div>{customer ? <div style={{ fontSize: 13, color: 'rgba(255,255,255,.85)', fontWeight: 700 }}>مرحباً، {customer.name} 👋</div> : <div style={{ fontSize: 12, color: 'rgba(255,255,255,.7)' }}>اطلب بالكارتون ووفّر أكثر</div>}<button onClick={() => setDrawerOpen(false)} style={{ position: 'absolute', top: 14, left: 14, background: 'rgba(255,255,255,.2)', border: 'none', color: 'white', width: 30, height: 30, borderRadius: '50%', cursor: 'pointer', fontSize: 15 }}>✕</button></div>
        <div className="drawer-nav">
          {[{ id: 'home', e: '🏠', l: 'الرئيسية' }, { id: 'search', e: '🔍', l: 'جميع المنتجات' }, { id: 'cats', e: '📂', l: 'الفئات والماركات' }, { id: 'promos', e: '🎯', l: 'العروض', b: promos.filter(x => x.active).length }, null, { id: 'wish', e: '❤️', l: 'المفضلة', b: wishlist.length }, { id: 'cart-d', e: '🛒', l: 'السلة', b: cartCount, a: () => setModal('cart') }, { id: 'track', e: '📍', l: 'تتبع الطلب', a: () => setModal('tracking') }, { id: 'quick', e: '⚡', l: 'الطلب السريع' }, null, { id: 'auth', e: '👤', l: customer ? customer.name : 'تسجيل الدخول', a: () => setModal(customer ? 'account' : 'login') }, { id: 'contact-d', e: '📞', l: 'اتصل بنا', a: () => setModal('contact') }, { id: 'dark', e: '🌙', l: 'الوضع الليلي', a: () => { document.body.classList.toggle('dark'); localStorage.setItem('nqDark', document.body.classList.contains('dark') ? '1' : '0') } }, { id: 'faq', e: '❓', l: 'الأسئلة الشائعة', a: () => setModal('faq') }, { id: 'terms', e: '📄', l: 'الشروط والأحكام', a: () => setModal('terms') }].map((it, i) => it === null ? <div key={i} className="di-div" /> : <div key={it.id} className={`di${tab === it.id && !it.a ? ' act' : ''}`} onClick={() => { (it.a ? it.a() : setTab(it.id)); setDrawerOpen(false) }}><div className="di-ico">{it.e}</div><span style={{ flex: 1 }}>{it.l}</span>{it.b > 0 && <span className="di-badge">{it.b}</span>}</div>)}
        </div>
      </div>
      {customer && (<div style={{ position: 'fixed', top: 78, left: 58, zIndex: 400, background: 'rgba(255,107,53,.9)', color: 'white', borderRadius: 20, padding: '5px 10px', fontSize: 11, fontWeight: 700, display: 'flex', gap: 6, alignItems: 'center' }}><span>👤 {customer.name}</span><button onClick={() => { setCustomer(null); localStorage.removeItem('nq_customer'); showToast('تم الخروج') }} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', fontSize: 13 }}>✕</button></div>)}
      <div className="page">{tabs[tab] || <Home />}</div>
      <div className="bnav">{[{ id: 'home', icon: 'fas fa-home', label: 'الرئيسية' }, { id: 'search', icon: 'fas fa-search', label: 'بحث' }, { id: 'promos', icon: 'fas fa-tag', label: 'العروض', badge: promos.filter(x => x.active).length }, { id: 'wish', icon: 'fas fa-heart', label: 'المفضلة', badge: wishlist.length }, { id: 'cart-m', icon: 'fas fa-shopping-basket', label: 'السلة', badge: cartCount, action: () => setModal('cart') }].map(b => (<button key={b.id} className={`bnav-b${tab === b.id && !b.action ? ' on' : ''}`} onClick={() => b.action ? b.action() : setTab(b.id)}><i className={b.icon}></i>{b.badge > 0 && <span className="nbadge">{b.badge}</span>}<span>{b.label}</span></button>))}</div>
      {modal === 'terms' && (<div className="moverlay" onClick={e => { if (e.target.className === 'moverlay') setModal(null) }}><div className="msheet"><div className="mhandle" /><div className="mhead"><h3>📄 الشروط والأحكام</h3><button className="mclose" onClick={() => setModal(null)}>✕</button></div><div className="mbody"><div style={{ fontSize: 14, lineHeight: 1.9, color: '#475569' }}>{settings['terms_text'] || `1. يُعدّ الطلب مؤكداً بعد التأكيد عبر واتساب فقط.\n2. الأسعار قابلة للتغيير دون إشعار مسبق.\n3. الطلب بالكارتون الكامل فقط.\n4. التوصيل يتم خلال 24-48 ساعة داخل الولاية.\n5. سياسة الاسترجاع: خلال 24 ساعة من الاستلام وفي حالة وجود عيوب مصنعية.`}</div></div></div></div>)}
      {modal === 'faq' && (<div className="moverlay" onClick={e => { if (e.target.className === 'moverlay') setModal(null) }}><div className="msheet"><div className="mhandle" /><div className="mhead"><h3>❓ الأسئلة الشائعة</h3><button className="mclose" onClick={() => setModal(null)}>✕</button></div><div className="mbody">{[{ q: 'ما هو الحد الأدنى للطلب؟', a: 'لا يوجد حد أدنى، يمكنك الطلب من كرتون واحد.' }, { q: 'كم تكلفة التوصيل؟', a: `التوصيل ${settings['shipping_cost'] || '500'} ${CUR}. مجاني للطلبات التي تتجاوز ${settings['free_shipping_threshold'] || '500'} ${CUR}.` }, { q: 'كيف أتتبع طلبي؟', a: 'اضغط على "تتبع الطلب" في القائمة وأدخل رقم هاتفك.' }, { q: 'كيف يتم التسليم؟', a: 'يتواصل معك فريقنا عبر واتساب لتحديد موعد التسليم.' }, { q: 'ما طرق الدفع المتاحة؟', a: 'الدفع نقداً عند الاستلام.' }, { q: 'هل يمكن الإلغاء بعد الطلب؟', a: 'يمكن الإلغاء قبل تأكيد الطلب عبر التواصل معنا.' }].map((item, i) => (<div key={i} style={{ marginBottom: 14, background: '#F7F3EF', borderRadius: 12, padding: 14 }}><div style={{ fontWeight: 800, fontSize: 14, color: '#1A0A00', marginBottom: 6 }}>❓ {item.q}</div><div style={{ fontSize: 13, color: '#475569', lineHeight: 1.6 }}>💡 {item.a}</div></div>))}</div></div></div>)}
      ...
      {modal === 'detail' && <DetailModal product={detailProd} wishlist={wishlist} onClose={() => setModal(null)} onAddCart={addToCart} onToggleWish={toggleWish} currency={CUR} products={products} onShowProduct={p => setDetailProd(p)} promos={promos} />}
