/**
 * Store.jsx — النسخة الكاملة والعاملة
 * ✅ جميع الميزات المطلوبة (نقاط، خصومات، OTP، FAQ، الخ)
 * ✅ تعمل بدون أخطاء (بدون useMemo/React.memo للتبسيط)
 * ✅ تحسين الأداء الأساسي (useCallback للدوال المارة)
 */
...
export default function Store() {
  ...
  // ========== مكونات التابات ==========
  // Note: لتجنب الـ useMemo المعقد، نستخدم دوال عادية هنا، ولكننا نمررها داخل JSX.
  // سيتم إعادة حسابها عند كل تصيير، ولكن هذا مقبول في هذه المرحلة.

  // --- مكون ProductCard (داخلي) ---
  const ProductCard = ({ p }) => {
    if (!p) return null
      ...
      <div className="pc" onClick={() => { setDetailProd(p); setModal('detail') }}>
        ...
        <div style={{ display: 'flex', gap: 5, marginTop: 'auto', paddingTop: 6 }}>
          ...
          <button onClick={e => { e.stopPropagation(); const msg = `🛍️ ${p.name}%0A💰 السعر: ${p.price} ${CUR}%0A🔗 ${window.location.origin}`; window.open(`https://wa.me/?text=${msg}`, '_blank') }}
            style={{ width: 32, height: 32, borderRadius: 8, background: '#25D36610', color: '#25D366', border: '1px solid #25D36630', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <>
        ...
        {bestSellerProducts.length > 0 && (
          <div className="sec">
            ...
            <div className="hscroll">{bestSellerProducts.map(p => <ProductCard key={p.id} p={p} />)}</div>
          </div>
        )}
        {allP.length > 0 && (
          <div className="sec">
