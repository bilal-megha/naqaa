...
export default function Employees() {
    ...
    try {
      if(editItem){
        ...
        await supabase.from('employees').insert({
          ...
          username:form.username,
          // bcrypt hash (آمن)
          password_hash: form.password ? await hashPassword(form.password) : undefined,
          password_algo: 'bcrypt',
          // حقل password القديم للتوافق المؤقت
          password: form.password ? await hashPassword(form.password) : undefined,
          email:form.email,
          phone:form.phone,
          role:'staff',
          permissions:JSON.stringify(form.permissions)
  ...
  const { paged: pagedEmployees, PagerUI } = usePagination(items, 12)
