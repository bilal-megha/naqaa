  ...
  const addItem = () => {
    ...
    if (existing) {
      ...
      setItems(prev => prev.map(i => i.productId == prod.id ? {
        ...i, cartons: newCartons, totalUnits: newCartons * parseInt(modal.unitsPerCarton),
      ...
      setItems(prev => [...prev, {
        ...
        totalUnits, purchasePrice: parseFloat(modal.purchasePrice), sellPrice: parseFloat(modal.sellPrice) || 0,
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (p) await supabase.from('products').update({
          ...
          cost_price: item.purchasePrice, carton_price: item.cartonPrice,
