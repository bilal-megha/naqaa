/**
 * @file pages/DataBackup.jsx
 * @description نسخ احتياطي واستعادة لكل بيانات المتجر (JSON)
 */
...
const BACKUP_TABLES = ['products', 'orders', 'customers', 'suppliers', 'brands', 'categories', 'purchases', 'coupons', 'expenses', 'notifications', 'settings']
const AUTO_BACKUP_TABLES = ['products', 'categories', 'brands', 'suppliers', 'customers', 'orders', 'purchases', 'expenses', 'promotions', 'settings']
...
export default function DataBackup({ showToast }) {
  ...
  const doAutoBackup = async () => {
    try {
      const backup = {}
      for (const t of AUTO_BACKUP_TABLES) {
        const { data } = await supabase.from(t).select('*').catch(() => ({ data: [] }))
        backup[t] = data || []
      }
      const blob = new Blob([JSON.stringify(backup, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url; a.download = `naqaa_auto_${new Date().toISOString().split('T')[0]}.json`; a.click()
  ...
  const backup = async () => {
    ...
    try {
      ...
      backupData._date = new Date().toISOString()
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginTop: 20, background: '#fef9c3', borderRadius: 12, padding: 14, fontSize: 13, color: '#92400e' }}>
          ⚠️ <strong>تنبيه:</strong> استعادة النسخة الاحتياطية ستضيف البيانات للموجودة ولن تحذف شيئاً. يُنصح بعمل نسخة احتياطية أسبوعية.
        </div>
