 ...
 * @description إرسال إشعارات مستهدفة للعملاء حسب الرتبة أو العنوان
 */
...
const TIER_LABELS = { all: 'الكل', M1: 'M1 عادي', M2: 'M2 مميز', M3: 'M3 VIP', address: 'حسب العنوان' }
const TIER_COLORS = { all: '#475569', M1: '#475569', M2: '#1d4ed8', M3: '#92400e', address: '#059669' }
  ...
  const send = async () => {
    ...
    try {
      await supabase.from('notifications').insert({
        id: Date.now(), title, body, target_type: targetType,
        target_count: targeted.length, date: new Date().toLocaleString('ar-DZ'), is_read: false,
      })
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
          {Object.entries(TIER_LABELS).map(([k, v]) => (
            <button key={k} onClick={() => setTargetType(k)}
              style={{ ...S.btnSm, background: targetType === k ? TIER_COLORS[k] : '#f1f5f9', color: targetType === k ? 'white' : '#64748b', border: `2px solid ${targetType === k ? TIER_COLORS[k] : 'transparent'}`, fontWeight: 700 }}>
              {v}
            </button>
          ))}
        </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  {n.target_type && n.target_type !== 'all' && (
                    <span style={{ fontSize: 11, background: '#f1f5f9', borderRadius: 20, padding: '2px 8px' }}>{TIER_LABELS[n.target_type] || n.target_type}</span>
                  )}
