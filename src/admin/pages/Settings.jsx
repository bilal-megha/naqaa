...
export default function Settings({ showToast }) {
  ...
  return (
    ...
    <div>
      ...
      {/* ══ معلومات الشركة للفواتير ══ */}
      <div style={{ ...S.card, marginTop: 16, border: '2px solid #1565C020' }}>
        <h3 style={{ fontWeight: 900, fontSize: 16, marginBottom: 6, color: '#1565C0' }}>🏢 معلومات الشركة (تظهر على الفواتير)</h3>
        <p style={{ fontSize: 12, color: '#64748b', marginBottom: 16 }}>هذه البيانات ستظهر تلقائياً على كل فاتورة تطبعها من POS أو المشتريات</p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={S.label}>اسم الشركة / RAISON SOCIALE *</label>
            <input style={S.input} value={form.company_name}
              onChange={e => setForm(f => ({ ...f, company_name: e.target.value }))}
              placeholder="مثال: SARL نقاء للمنظفات والتوزيع" />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label style={S.label}>العنوان / ADRESSE</label>
            <input style={S.input} value={form.company_address}
              onChange={e => setForm(f => ({ ...f, company_address: e.target.value }))}
              placeholder="مثال: المنطقة الصناعية، تقيدوت، واد الشعبة، باتنة" />
          </div>
          <div>
            <label style={S.label}>الهاتف / TEL-FAX</label>
            <input style={S.input} value={form.company_phone}
              onChange={e => setForm(f => ({ ...f, company_phone: e.target.value }))}
              placeholder="مثال: 033 XX XX XX" />
          </div>
          <div>
            <label style={S.label}>الجوال / MOB</label>
            <input style={S.input} value={form.company_mobile}
              onChange={e => setForm(f => ({ ...f, company_mobile: e.target.value }))}
              placeholder="مثال: 0550033200" />
          </div>
          <div>
            <label style={S.label}>البريد الإلكتروني / E-MAIL</label>
            <input style={S.input} value={form.company_email}
              onChange={e => setForm(f => ({ ...f, company_email: e.target.value }))}
              placeholder="exemple@mail.com" />
          </div>
          <div>
            <label style={S.label}>رأس المال / Capital Soc.</label>
            <input style={S.input} value={form.company_capital}
              onChange={e => setForm(f => ({ ...f, company_capital: e.target.value }))}
              placeholder="مثال: 20.000.000,00" />
          </div>
        </div>

        {/* المعرفات الجبائية */}
        <div style={{ marginTop: 14, padding: '12px 16px', background: '#fffbeb', borderRadius: 12, border: '1px solid #fcd34d' }}>
          <div style={{ fontWeight: 800, fontSize: 13, color: '#92400e', marginBottom: 12 }}>🏛️ المعرفات الجبائية</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
            <div>
              <label style={S.label}>RC N°</label>
              <input style={S.input} value={form.company_rc}
                onChange={e => setForm(f => ({ ...f, company_rc: e.target.value }))}
                placeholder="مثال: 05/05-0303937 B 22" />
            </div>
            <div>
              <label style={S.label}>ART N°</label>
              <input style={S.input} value={form.company_art}
                onChange={e => setForm(f => ({ ...f, company_art: e.target.value }))}
                placeholder="مثال: 05160106455" />
            </div>
            <div>
              <label style={S.label}>NIF N°</label>
              <input style={S.input} value={form.company_nif}
                onChange={e => setForm(f => ({ ...f, company_nif: e.target.value }))}
                placeholder="مثال: 002217030393..." />
            </div>
            <div>
              <label style={S.label}>NIS N°</label>
              <input style={S.input} value={form.company_nis}
                onChange={e => setForm(f => ({ ...f, company_nis: e.target.value }))}
                placeholder="NIS..." />
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: 14 }}>
          <label style={S.label}>رقم الفاتورة التالي (ترقيم تلقائي)</label>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <input type="number" style={{ ...S.input, width: 150 }} value={form.invoice_next_num}
              onChange={e => setForm(f => ({ ...f, invoice_next_num: e.target.value }))} />
            <span style={{ fontSize: 13, color: '#64748b' }}>
              ← الفاتورة القادمة: <strong style={{ color: '#1565C0' }}>
                {String(parseInt(form.invoice_next_num) || 1).padStart(6, '0')}/{new Date().getFullYear().toString().slice(-2)}
              </strong>
            </span>
          </div>
        </div>
        ...
        {/* معاينة الفاتورة */}
        {form.company_name && (
          <div style={{ marginTop: 16, padding: 16, background: 'white', border: '2px solid #1E293B', borderRadius: 10 }}>
            <div style={{ textAlign: 'center', borderBottom: '1px solid #ccc', paddingBottom: 10, marginBottom: 10 }}>
              <div style={{ fontWeight: 900, fontSize: 16 }}>{form.company_name}</div>
              {form.company_address && <div style={{ fontSize: 12 }}>{form.company_address}</div>}
              {(form.company_phone || form.company_mobile) && (
                <div style={{ fontSize: 11, color: '#475569' }}>
                  TEL: {form.company_phone} &nbsp;|&nbsp; MOB: {form.company_mobile}
                  {form.company_email && <span> &nbsp;|&nbsp; {form.company_email}</span>}
                </div>
              )}
              {(form.company_rc || form.company_nif) && (
                <div style={{ fontSize: 10, color: '#64748b', marginTop: 4 }}>
                  {form.company_rc && `RC: ${form.company_rc}`}
                  {form.company_art && ` | ART: ${form.company_art}`}
                  {form.company_nif && ` | NIF: ${form.company_nif}`}
                  {form.company_nis && ` | NIS: ${form.company_nis}`}
                </div>
              )}
            </div>
            <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>← معاينة رأس الفاتورة</div>
          </div>
        )}
      </div>
