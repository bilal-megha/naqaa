 ...
 * @description إدارة الموردين مع سلة المهملات
 */
...
export default function Suppliers() {
  ...
  const save = async () => {
    ...
    try {
      ...
      await logActivity(form.id ? 'تعديل مورد' : 'إضافة مورد', `${form.id ? 'تم تعديل' : 'تم إضافة'} المورد: ${form.name}`)
  ...
  const del = async id => { await softDelete('suppliers', id, items, setItems, load, showToast, askConfirm) }
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={S.grid2}>
          ...
          <div><label style={S.label}>واتساب</label><PhoneInput value={form.whatsapp} onChange={F('whatsapp')} placeholder="مثال: 0555123456" /></div>
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.id} className="nq-tr">
                  ...
                  <td style={S.td}>
                    {s.whatsapp
                      ? <a href={`https://wa.me/${s.whatsapp}`} target="_blank" style={{ color: '#25D366', fontWeight: 700 }}>💬 {s.whatsapp}</a>
                      : '—'}
                  </td>
