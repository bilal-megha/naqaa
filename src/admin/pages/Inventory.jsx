...
export default function Inventory() {
  ...
  const printInventory = () => {
    printA4(`
      ...
      <tbody>${pagedInventory.map(p=>`<tr><td>${p.name}</td><td>${p.sku||'—'}</td><td>${p.stock||0}</td><td>${p.min_stock||5}</td><td>${(p.stock||0)<(p.min_stock||5)?'⚠️ منخفض':(p.stock||0)<20?'متوسط':'جيد'}</td><td>${((p.stock||0)*Number(p.price)).toFixed(0)} ${CUR}</td></tr>`).join('')}</tbody></table>
            {PagerUI}
      <div class="footer">إجمالي قيمة المخزون: ${filtered.reduce((s,p)=>s+(p.stock||0)*Number(p.price),0).toFixed(0)} ${CUR}</div>
    `)
  }

  const { paged: pagedInventory, PagerUI } = usePagination(filtered, 15)
