 ...
 * @description إدارة الطلبيات — خريطة، تجميع بالعنوان، أرشفة، طباعة، Pagination
 */
import { useState, useEffect, useCallback } from 'react'
import { supabase } from '../../lib/supabase.js'
import { CLR, S, CUR } from '../constants.js'
import { logActivity, printThermal, printA4 } from '../utils.js'
...
export default function Orders() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      showToast('❌ خطأ في تحميل الطلبيات', 'error')
    }
  }, [page, statusFilter])
  ...
  const updateMulti = async status => {
    ...
    try {
      ...
      setSelectedOrders([]); await load()
    } catch (err) { showToast('❌ خطأ: ' + err.message, 'error') }
  }

  /** أرشفة الطلبيات المسلّمة الأقدم من 6 أشهر لتحسين الأداء */
    ...
    <div>{ToastUI}
      ...
      <div style={{ ...S.card, background: '#F0FDF4', border: '1px solid #A7F3D0', marginBottom: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
          ...
          <button onClick={archiveOldOrders} style={{ ...S.btn, background: '#059669', fontSize: 13 }}>📦 أرشفة الآن</button>
        </div>
      </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginTop: 10, fontSize: 13, color: CLR.textSm }}>
          {totalCount} طلبية إجمالاً — {filtered.length} معروضة — إجمالي: {filtered.reduce((s, o) => s + Number(o.total), 0).toFixed(0)} {CUR}
        </div>
      </div>
            ...
            {zOrders.map(o => (
              <div key={o.id} style={{ background: '#f8fafc', borderRadius: 12, padding: 12, marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  {o.customer_phone && (
                    <a href={`https://wa.me/${o.customer_phone.replace(/^0/, '213')}?text=مرحباً ${o.customer_name}، طلبكم رقم ${o.id} جاهز للتوصيل`} target="_blank"
                  ...
                  <tr key={o.id} className="nq-tr">
                    ...
                    <td style={S.td}>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        ...
                        <button style={{ ...S.btnSm, background: '#f0fdf4', color: '#059669' }} onClick={() => printReceipt(o)}>🖨️</button>
                        {o.customer_phone && (
                          <a href={`https://wa.me/${o.customer_phone.replace(/^0/, '213')}?text=مرحباً ${o.customer_name}، طلبكم رقم ${o.id} في الطريق`} target="_blank"
                            style={{ ...S.btnSm, background: '#dcfce7', color: '#059669', textDecoration: 'none', padding: '5px 10px' }}>💬</a>
                        )}
                      </div>
                    </td>
