/**
 * @file pages/Promotions.jsx
 * @description إدارة العروض الترويجية (خصم نسبة، مبلغ، اشتري واحصل، خصم حسب الكمية)
 */
...
const EMPTY_FORM = {
  id: '', name: '', type: 'percent', active: true, buy_qty: 3, get_qty: 1,
  discount_value: 0, product_ids: [], min_amount: 0, description: '', end_date: '',
  image: '', tier_qty: 1, tier_type: 'percent', tier_value: 0, region: '',
}

const TYPE_LABEL = {
  percent: 'خصم نسبة %', fixed: 'خصم مبلغ ثابت', buy_x_get_y: 'اشتري X خذ Y',
  tier_discount: 'خصم حسب الرتبة', tier_buy: 'اشتري X من نفس الشركة = خصم',
}
...
export default function Promotions() {
  ...
  const toggleProduct = id => setForm(f => ({
    ...f, product_ids: f.product_ids.includes(id) ? f.product_ids.filter(x => x !== id) : [...f.product_ids, id],
  }))
  const handleImg = e => {
  ...
  const save = async () => {
    ...
    try {
      const row = {
        id: form.id || Date.now(), name: form.name.trim(), type: form.type,
        ...
        min_amount: parseFloat(form.min_amount) || 0, description: form.description || '',
        image: form.image || null, end_date: form.end_date ? new Date(form.end_date).toISOString() : null,
        tier_qty: parseInt(form.tier_qty) || 1, tier_type: form.tier_type || 'percent',
        tier_value: parseFloat(form.tier_value) || 0, region: form.region || null,
      }
      ...
      await logActivity(form.id ? 'تعديل عرض' : 'إضافة عرض', `${form.id ? 'تم تعديل' : 'تم إضافة'} العرض: ${form.name}`)
  ...
  const edit = p => setForm({
    ...
    min_amount: p.min_amount || 0, description: p.description || '',
    end_date: p.end_date?.split('T')[0] || '', image: p.image || '', region: p.region || '',
    tier_qty: p.tier_qty || 1, tier_type: p.tier_type || 'percent', tier_value: p.tier_value || 0,
  })

  const del = async id => { await softDelete('promotions', id, promos, setPromos, load, showToast, askConfirm) }
  ...
  const visibleProducts = products.filter(p => !prodSearch || p.name.toLowerCase().includes(prodSearch.toLowerCase()))
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={S.grid2}>
          ...
          <div><label style={S.label}>المنطقة</label><input style={S.input} value={form.region} onChange={F('region')} placeholder="مثال: الجزائر العاصمة" /></div>
        </div>

        {form.type === 'percent' && (
          <div style={{ marginTop: 12 }}><label style={S.label}>نسبة الخصم %</label><NumInput value={form.discount_value} onChange={F('discount_value')} placeholder="مثال: 20" /></div>
        )}
        {form.type === 'fixed' && (
          <div style={{ marginTop: 12 }}><label style={S.label}>مبلغ الخصم ({CUR})</label><NumInput value={form.discount_value} onChange={F('discount_value')} /></div>
        )}
        {form.type === 'buy_x_get_y' && (
          <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
            <div><label style={S.label}>اشتري كم؟</label><NumInput value={form.buy_qty} onChange={F('buy_qty')} /></div>
            <div><label style={S.label}>خذ كم مجاناً؟</label><NumInput value={form.get_qty} onChange={F('get_qty')} /></div>
            <div style={{ padding: '14px 0', fontSize: 14, color: CLR.textSm, alignSelf: 'flex-end' }}>← أي منتج من الأرخص يكون مجاناً</div>
          </div>
        )}
        ...
        {form.type === 'tier_buy' && (
          <div style={{ background: '#f0f9ff', borderRadius: 12, padding: 14, marginTop: 12 }}>
            ...
            <p style={{ fontSize: 12, color: CLR.textSm, marginTop: 8 }}>
              مثال: اشتري {form.tier_qty} كرتون من نفس الشركة → {form.tier_value}{form.tier_type === 'percent' ? '%' : ' ' + CUR} خصم
            </p>
          </div>
        )}
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            ...
            <button style={{ ...S.btnSm, background: CLR.accent, color: 'white', padding: '6px 14px' }} onClick={selectAllVisible}>تحديد الكل</button>
            {form.product_ids.length > 0 && (
              <button style={{ ...S.btnSm, background: '#FEE2E2', color: CLR.danger }} onClick={() => setForm(f => ({ ...f, product_ids: [] }))}>إلغاء الكل</button>
            )}
          </div>
              ...
              visibleProducts.map((p, i) => {
                ...
                return (
                  <label key={p.id} onClick={() => toggleProduct(p.id)}
                    style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', cursor: 'pointer', background: sel ? '#FFF7ED' : i % 2 === 0 ? 'white' : CLR.bg, borderBottom: `1px solid ${CLR.border}` }}>
                    <input type="checkbox" checked={sel} onChange={() => {}} style={{ accentColor: CLR.accent, width: 16, height: 16, cursor: 'pointer' }} />
                    {p.image
                      ? <img src={p.image} style={{ width: 38, height: 38, borderRadius: 8, objectFit: 'cover', border: `1px solid ${CLR.border}` }} />
                      : <div style={{ width: 38, height: 38, borderRadius: 8, background: CLR.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>📦</div>}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          ...
          <button style={S.btnGray} onClick={() => setForm(EMPTY_FORM)}>✖ إلغاء</button>
        </div>
      </div>
            ...
            <div key={p.id} style={{ background: p.active && !isExpired ? '#f0fdf4' : '#f8fafc', borderRadius: 14, padding: 14, marginBottom: 10, border: `1px solid ${p.active && !isExpired ? '#10b981' : '#e2e8f0'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: CLR.textSm, marginTop: 2 }}>{TYPE_LABEL[p.type] || p.type}</div>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 700, background: p.active && !isExpired ? '#d1fae5' : isExpired ? '#fee2e2' : '#fef9c3', color: p.active && !isExpired ? '#059669' : isExpired ? '#dc2626' : '#92400e' }}>
                    {isExpired ? 'منتهي' : p.active ? '✅ فعّال' : '⏸️ موقوف'}
                  </span>
                  <button style={{ ...S.btnSm, background: p.active ? '#fef9c3' : '#d1fae5', color: p.active ? '#92400e' : '#059669' }} onClick={() => toggleActive(p.id, !p.active)}>
