...
import { softDelete, logActivity } from '../styles/helpers.js'
  ...
  const load = async () => {
    try {
      const [{ data: p }, { data: pr }] = await Promise.all([
        ...
        supabase.from("promotions").select("*").is('deleted_at', null).order("id", { ascending: false }),
      ]);
      setProducts(p || []);
      setPromos(pr || []);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={S.grid2}>
          ...
          <div>
            ...
            <select style={S.input} value={form.type} onChange={F("type")}>
              ...
              <option value="tier_discount">خصم حسب الرتبة</option>
            </select>
          </div>
          <div>
        ...
        {form.type === "tier_discount" && (
          <div style={{ marginTop: 12 }}>
            <div style={S.grid2}>
              <div>
                <label style={S.label}>الكمية المؤهلة</label>
